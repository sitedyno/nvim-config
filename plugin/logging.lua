...
vim.api.nvim_create_autocmd('CmdlineEnter', {
    ...
    callback = function (_)
        ...
        complete = function (arg_lead, cmdline, _)
            ...
            if subcmd_key and subcmd_arg_lead and commands_tbl[subcmd_key] and commands_tbl[subcmd_key].complete then
                return commands_tbl[subcmd_key].complete(subcmd_arg_lead)
            end
            if cmdline:match("^['<,'>]*Sd[!]*%s+%w*$") then
                local keys = vim.tbl_keys(commands_tbl)
                return vim.iter(keys):filter(function(k) return k:find(arg_lead) ~= nil end):totable()
            end
        end
    })
    end,
